...
public class ParfaitAgent {
    ...
    public static void startView(MBeanServerConnection server) throws InstanceNotFoundException, IntrospectionException, AttributeNotFoundException, UnsupportedOperationException, ReflectionException, MBeanException, IOException {
        AgentMonitoringView view = new AgentMonitoringView(server);

        List<Specification> jsonSpecifications;
        List<Specification> mBeansSpecifications;

        jsonSpecifications = parseAllSpecifications();
        mBeansSpecifications = SpecificationMBeans.gatherMBeansData(server);

        for (Specification specification : jsonSpecifications) {
            logger.trace(String.format("Registering JSON-source metric [%s]", specification.getName()));
            view.register(specification);
        }

        for (Specification specification : mBeansSpecifications) {
            logger.trace(String.format("Registering JMX-source metric [%s]", specification.getName()));
            view.register(specification);
        }
